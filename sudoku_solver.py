...
"""
    sudoku_solver.py
    2020
    Uses a simple brute-force backtracking algorithm.
"""
...
class Board:
    ...
    def __getitem__(self, coord):
        return self._data[coord[0]][coord[1]]

    def __setitem__(self, coord, value):
        if self.is_available(coord):
            self._data[coord[0]][coord[1]] = value
        else:
    ...
    def is_available(self, coord):
        ...
        return coord not in self._static_digits_index
    ...
    def get_digits_available(self, coord):
        ...
        digits_in_row = set(self._data[ii][x] for x in range(9))
        digits_in_column = set(self._data[y][jj] for y in range(9))
...
def next_coord(coord):
    """ Returns `next` coordinate (i, j) in grid. 
            (... -> (1, 8) -> (2, 0) -> (2, 1) -> ...)
        If coord is last (8, 8), it returns (9, 0) as opposed to wrapping around to (0, 0).
    """
